...
local triggerCurrentCompletion = function(bufnr, line_to_cursor, prefix, textMatch, suffix, force)
  ...
  if manager.insertChar == false then
    return
  end
  ...
  local triggered
  triggered = util.checkTriggerCharacter(line_to_cursor, source_trigger_character)
    or util.checkTriggerCharacter(line_to_cursor, opt.get_option 'trigger_character')
  ...
  complete.performComplete(
    complete_source,
    complete_items_map,
    { bufnr = bufnr, prefix = prefix, textMatch = textMatch, suffix = suffix, line_to_cursor = line_to_cursor }
  )
end
