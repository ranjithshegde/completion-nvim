...
local function get_completion_word(item, prefix, suffix)
    if
        item.textEdit ~= nil
        and item.textEdit ~= vim.NIL
        and item.textEdit.newText ~= nil
        and (item.insertTextFormat ~= 2 or vim.fn.exists "g:loaded_vsnip_integ")
    then
        ...
        if
            ...
            or opt.get_option "enable_snippet" == "snippets.nvim"
        then
            return newText
        else
...
local function text_document_completion_list_to_complete_items(result, params)
    ...
    for _, completion_item in ipairs(items) do
        ...
        if
            protocol.InsertTextFormat[completion_item.insertTextFormat] == "Snippet"
            and opt.get_option "enable_snippet" == "snippets.nvim"
        then
