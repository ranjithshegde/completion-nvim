...
M.autoOpenSignatureHelp = function()
    ...
    for _, value in pairs(vim.lsp.buf_get_clients(0)) do
        if
            value.resolved_capabilities.signature_help == false
            or value.server_capabilities.signatureHelpProvider == nil
        then
            return
        end

        line_to_cursor = vim.trim(line_to_cursor)
        triggered = util.checkTriggerCharacter(
            line_to_cursor,
    ...
    if triggered then
        ...
        vim.lsp.buf_request(0, "textDocument/signatureHelp", params, function(err, result, context)
            ...
            local fbuf, fwin = vim.lsp.util.open_floating_preview(lines, trimmed_lines_filetype, opts)
            if hl then
